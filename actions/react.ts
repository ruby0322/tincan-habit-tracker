...
const reactToPost = async (user_id: string, post_id: string, reaction_type: string): Promise<boolean> => {
        ...
        if(!data || data.length == 0){//haven't react before
           const { error } = await supabase
            .from('react_to')
            .insert({ 'user_id': user_id, 'post_id': post_id, 'reaction_type': reaction_type }) ;
            if (error) {
                throw new Error(`Error reacting to a post: ${error.message}`);
            }
        }
        else{//have react before
            const existingReaction = data[0];
            console.log("hey = ", existingReaction)
            if(existingReaction.reaction_type == reaction_type){//delete reaction
                const { error } = await supabase
                    .from('react_to')
                    .delete()
