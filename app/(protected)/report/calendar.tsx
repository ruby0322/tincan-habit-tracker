  ...
  // }, []);
    ...
    return (
        <div className="">
                ...
                <ResizablePanel defaultSize={20}>
                    <div className="flex h-full items-center justify-between p-5">
                        {weekDays.map((weekDay, index) => (
                            <div key={index} className={index === 0 || index === 6 ? "text-red-500 text-center" : "text-center"}>{weekDay}</div>
                        ))}
                    </div>
                </ResizablePanel>
                <ResizableHandle />
            ...
            <div className="px-4 text-xl italic font-bold">{progress.toString() + "%"}</div>
        </div>
        </div>
    )
}
